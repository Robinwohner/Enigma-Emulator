    ...
    const int N)
{
    ...
    while (true)
    {
        ...
        for (int i = 0; i < P; i++)
        {                
            prompt = get_string("Select two letters for pair no. %d: \n", i + 1);

            //check if two letters were entered
            if (strlen(prompt) != 2)
            {
                b = true;
                printf("ERROR: Two letters are required for a plugboard connection. Retry\n");
                break;
            }
            
            plug[i][0] = toupper(prompt[0]);
            plug[i][1] = toupper(prompt[1]);
